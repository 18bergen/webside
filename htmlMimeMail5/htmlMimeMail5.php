...
class htmlMimeMail5
{
    ...
    */
    public function __construct()
    {
        ...
        */
        ...
        $this->build_params['text_encoding'] = new QPrintEncoding(); //new SevenBitEncoding();
        $this->build_params['html_charset']  = 'UTF-8'; // ISO-8859-1
        $this->build_params['text_charset']  = 'UTF-8'; // ISO-8859-1
        $this->build_params['head_charset']  = 'UTF-8'; // ISO-8859-1
        ...
        */
        ...
        $this->headers['X-Mailer'] = 'htmlMimeMail5 <http://www.18bergen.org/>';
    }
    ...
    */
    ...
    /**
    * Function to encode subject (added by Dan Michael O. Heggø)
    * according to RFC2047
    * 
    * @param  string $input   Value to encode
    * @param  string $charset Character set to use
    * @return string          Encoded value
    */
    private function encodeSubjectHeader($input, $charset = 'ISO-8859-1')
    {
        preg_match_all('/(\w*[\x80-\xFF]+\w*)/', $input, $matches);
        foreach ($matches[1] as $value) {
            $replacement = preg_replace('/([\x80-\xFF])/e', '"=" . strtoupper(dechex(ord("\1")))', $value);
        ...
        // Slight modification by Dan Michael :)
        ...
        switch ($type) {
            case 'mail':
                ...
                if (!empty($this->headers['Subject'])) {
                    $subject = $this->encodeSubjectHeader($this->headers['Subject'], $this->build_params['head_charset']);
